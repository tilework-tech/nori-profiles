...
Path: @/plugin/src/installer/features/profiles/config/_mixins/_docs/subagents
...
These subagent .md files are copied to ~/.claude/agents/ during profile installation by @/plugin/src/installer/features/subagents/. The main agent invokes these subagents using the Task tool with a subagent_type parameter. The /initialize-noridocs slash command (defined in @/plugin/src/installer/features/profiles/config/_mixins/_docs/slashcommands/) invokes nori-initial-documenter, while /update-noridocs invokes nori-change-documenter. Both subagents integrate with the sync-noridocs skill (from @/plugin/src/installer/features/skills/config/paid/meta/sync-noridocs/) to push documentation to remote servers. These subagents are referenced in profile CLAUDE.md files (like @/plugin/src/installer/features/profiles/config/amol/CLAUDE.md) as part of the documentation workflow.
