...
- `install.ts` exposes `main` (wraps `noninteractive` with optional silent-mode output suppression) and `noninteractive` (init via `ensureNoriInitialized`, resolve skillset, then `completeInstallation`). Both accept an optional `persistActiveSkillset` flag (default: persist). Persisting the global `activeSkillset` to `.nori-config.json` via `updateConfig` is now gated on `persistActiveSkillset !== false`: a transient `--install-dir` switch passes `false` so it never clobbers the user's global active skillset, but the selected skillset is still threaded in-memory into `completeInstallation`'s config (`{ ...config, installDir, activeSkillset: selectedSkillset, persistActiveSkillset: false }`) so the correct files still land in the target dir — and so `configLoader` (which otherwise writes `config.activeSkillset` to disk) keeps the on-disk global value.
- `completeInstallation` claims the process's install-lifecycle context through @/src/cli/installTracking.ts before creating the progress marker. The claim is one-use: it reuses any first-install or update context opened by the CLI entrypoint, otherwise classifies this explicit operation as a reinstall, and later nested install calls receive no second context. The same activity is completed with `success` after `installSkillset`, or with `failure` in the catch path before the original error is rethrown. Tracking never replaces cleanup, banners, or installer error behavior.
...
- `noninteractive` exits with status 1 when configuration or a skillset cannot be resolved, so callers on fresh installs must pass `skillset` explicitly. These direct exits first complete any unclaimed lifecycle, report the active allowlisted command failure, and perform the same bounded analytics flush as framed failures.
...
- Reinstall classification applies only because this path explicitly requested installation. Ordinary CLI startup at the same or lower version remains analytics-silent and does not move install state backward.
