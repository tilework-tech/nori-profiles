...
describe("profilesLoader", () => {
  ...
  describe("run", () => {
    it("should create profiles directory and copy profile templates for free installation", async () => {
      ...
      expect(files).toContain("onboarding-wizard");
      expect(files).not.toContain("_base"); // _base is never installed
    });

    it("should install onboarding-wizard profile with correct structure", async () => {
      const config: Config = {
        installDir: tempDir,
        agents: {
          "claude-code": { profile: { baseProfile: "senior-swe" } },
      ...
      // Verify onboarding-wizard profile exists
      const wizardPath = path.join(profilesDir, "onboarding-wizard");
      ...
      expect(profileJson.name).toBe("onboarding-wizard");
      expect(profileJson.builtin).toBe(true);
      expect(profileJson.description).toContain("wizard");
      ...
      expect(claudeMdContent).toContain("Onboarding Wizard");
    });
