...
/**
 * Clear hooks from settings.json before update to prevent stale hook references.
...
const clearHooksBeforeUpdate = async (): Promise<void> => {
  ...
  try {
    ...
    if (settings.hooks != null) {
      ...
      debug({ message: "Cleared hooks from settings.json before autoupdate" });
    }
  } catch {
    // Gracefully handle missing or malformed settings.json
    // The update should proceed regardless
    debug({
      message: "Could not clear hooks from settings.json (file may not exist)",
    });
  }
};
