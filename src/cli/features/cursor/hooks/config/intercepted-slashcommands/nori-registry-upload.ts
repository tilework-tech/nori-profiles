/**
 * Cursor intercepted slash command for uploading profiles to the registry
 * Handles /nori-registry-upload <profile-name> [version] [registry-url] command
 * Duplicate of Claude Code version - this command is IDE-agnostic
 */
...
const run = async (args: { input: HookInput }): Promise<HookOutput | null> => {
  ...
  // Check profile exists - CRITICAL: Use Cursor profiles directory, not Claude
...
/**
 * Cursor nori-registry-upload intercepted slash command
 */
export const cursorNoriRegistryUpload: InterceptedSlashCommand = {
  matchers: [
