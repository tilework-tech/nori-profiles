...
- `_base`: Core infrastructure (using-skills, creating-skills, web-search-researcher, nori-debug/info/nori-switch-profile/nori-toggle-autoupdate commands). The creating-skills skill provides an interactive workflow for users to create custom skills, guiding them through requirements gathering, profile selection, directory creation, SKILL.md writing, optional script bundling, verification, and profile switching. The nori-toggle-autoupdate command toggles the autoupdate setting in `.nori-config.json` between 'enabled' and 'disabled', allowing users to control whether Nori Profiles automatically updates on session start.
