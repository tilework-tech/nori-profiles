...
- `switchSkillset`: Validates the target skillset exists (has `nori.json`) and logs success. Does not persist config. Resolves the target directory for a bare name across storage buckets via `resolveSkillsetDir` from @/src/norijson/skillset.ts (falling back to the legacy flat path).
...
**Legacy skill resolver** (`skillResolver.ts`): The pre-`nori.json` `skills.json` manifest layer. Only `addSkillDependency` has a production caller (`skill-download`, which writes the resolved exact version into `skills.json`); nothing reads `skills.json` back at runtime. `resolveSkillVersion` is the repo's only caller of `semver.satisfies` and has no production callers at all — there is no version-range resolution anywhere in the install path, which is precisely why `nori.json` dependency entries are recorded unpinned (see @/src/norijson/docs.md).
