...
- `preflightSkillsetRemovalAtExactInstallDir` / `removeSkillsetAtExactInstallDir`: The deliberately narrower two-phase removal path used by `clear --exact-install-dir`. Preflight requires a valid keyed manifest for every marked target, validates its recorded install directory, confines every deletion lexically and through real paths, and reads the instructions update before any mutation. Removal then applies that immutable plan. The path does not read legacy manifests, run loader uninstall hooks, or restore external/home-level settings, so clearing a workspace layer cannot mutate parent or global agent state.
...
**Neither pass treats a fenced code block specially.** Path placeholders and agent conditionals inside a ``` block are both expanded, exactly as they would be in prose. This is a deliberate symmetry: published skillsets put runnable commands like `node {{skills_dir}}/x.js` in fences and depend on the path resolving, and having conditionals follow a different rule meant authors had to remember which construct a fence protected. Backtick-wrapping a tag is the single escape hatch for both. The cost is that a document cannot demonstrate this syntax in a fence without being consumed by it — which is why the `syntax` command's reference is CLI output rather than shippable skill content.
...
- **Fence-skipping was tried and removed.** An earlier revision skipped fences during conditional expansion so that documentation about the syntax would survive its own installer. Extending that to path placeholders was measured against every markdown file in the local profile library and changed a large fraction of them, including `bash` blocks whose whole purpose is to be runnable — so the two passes would have had to disagree about what a fence means. Uniform treatment won; the escape hatch is backticks.
