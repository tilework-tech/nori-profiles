...
- Check git status - are you on main?
  ...
  - If creating a worktree, read and follow `{{rules_dir}}/using-git-worktrees/RULE.md` to automatically create a worktree. Derive the branch name from my request.
...
- Ask if me wants to follow test driven development.
  - If yes: use test driven development. Read and follow `{{rules_dir}}/test-driven-development/RULE.md`
- Ask if me wants to update docs.
  - If yes: update documentation if the codebase uses docs.md files (noridocs).
- Ask if me wants to create a PR.
  - If yes: read and follow `{{rules_dir}}/finishing-a-development-branch/RULE.md`
</required>
...
<system-reminder>noridocs are docs.md files colocated with folders in the codebase.</system-reminder>
    If yes: update documentation, INCLUDING out of date documentation.
