...
// Mock cursor-agent paths
vi.mock("@/cli/features/cursor-agent/paths.js", () => ({
  getCursorHomeHooksFile: vi.fn(() => "/home/user/.cursor/hooks.json"),
}));
...
describe("autoupdate", () => {
  describe("E2E integration tests", () => {
    ...
    it("should log install output via Winston debug", async () => {
      ...
      // Mock fs/promises for hooks.json (no hooks to clear)
  ...
  describe("stale hooks prevention", () => {
    ...
    it("should clear hooks from hooks.json before spawning npm install", async () => {
      // This test verifies the fix for stale hooks: when autoupdate runs,
      // it should remove hooks from hooks.json BEFORE npm install replaces files.
      ...
      // Mock loadConfig with autoupdate enabled (cursor-agent defaults to enabled)
      const { loadConfig } = await import("@/cli/config.js");
          ...
          SessionEnd: [
            {
              ...
              hooks: [
                {
                  type: "command",
                  command: "node /old/path/to/hook.js",
    ...
    it("should handle missing settings.json gracefully when clearing hooks", async () => {
      ...
      // Mock loadConfig (cursor-agent defaults to enabled)
      const { loadConfig } = await import("@/cli/config.js");
      vi.mocked(loadConfig).mockResolvedValue({
        auth: null,
        agents: null,
