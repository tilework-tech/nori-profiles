...
# This script is called by Cursor Agent hooks on the stop event
# Supports Linux (notify-send), macOS (osascript), and Windows (PowerShell)

# Get install directory from environment variable (set by hooks.json command)
# Falls back to deriving from script location if not set (for testing/debugging)
if [ -n "$NORI_INSTALL_DIR" ]; then
    INSTALL_DIR="$NORI_INSTALL_DIR"
else
