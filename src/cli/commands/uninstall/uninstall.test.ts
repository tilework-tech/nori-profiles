...
describe("uninstall directory display", () => {
  let tempDir: string;
  let originalHome: string | undefined;
  let processExitSpy: any;
  ...
  beforeEach(async () => {
    originalHome = process.env.HOME;
    tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "uninstall-dir-display-"),
    );
    process.env.HOME = tempDir;
  ...
  it("should display directory at start and end during interactive uninstall", async () => {
    // Create a Nori installation
    await fs.writeFile(
    ...
    // Mock prompts: confirm uninstall, confirm global settings removal
    (promptUser as any).mockResolvedValueOnce("y"); // Confirm uninstall
    (promptUser as any).mockResolvedValueOnce("y"); // Remove global settings

    // Import the mocked info function
    const { info } = await import("@/cli/logger.js");

    // Run interactive uninstall
    await main({ nonInteractive: false, installDir: tempDir });

    // Verify info was called with "Uninstalling from:" at the start
    const infoCalls = (info as any).mock.calls.map(
      (call: any) => call[0].message,
    );
    const uninstallingFromMessages = infoCalls.filter((msg: string) =>
      msg.includes("Uninstalling from:"),
    );
    expect(uninstallingFromMessages.length).toBeGreaterThanOrEqual(1);
    expect(uninstallingFromMessages[0]).toContain(tempDir);

    // Verify info was called with "Uninstalled from:" at the end
    const uninstalledFromMessages = infoCalls.filter((msg: string) =>
      msg.includes("Uninstalled from:"),
    );
    expect(uninstalledFromMessages.length).toBeGreaterThanOrEqual(1);
    expect(uninstalledFromMessages[0]).toContain(tempDir);
  });

  it("should display directory at start and end during non-interactive uninstall", async () => {
    // Create a Nori installation
    await fs.writeFile(
      path.join(tempDir, ".nori-config.json"),
      JSON.stringify({}),
    ...
    // Run non-interactive uninstall
    await main({ nonInteractive: true, installDir: tempDir });
