...
describe("--skillset option and manifest updates", () => {
  ...
  it("should error when specified skillset does not exist", async () => {
    ...
    const clackErrorOutput = getClackErrorOutput();
    expect(clackErrorOutput.toLowerCase()).toContain("not found");
    expect(clackErrorOutput).toContain("nonexistent-profile");
