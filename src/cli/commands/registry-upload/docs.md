...
This is the **only** path that writes exact versions into `dependencies.skills` / `dependencies.subagents`. Every consumer-side install normalizes those same entries back to the unpinned `"*"` specifier, so a publisher's local manifest and a freshly installed copy of the same skillset legitimately differ in their dependency values. See the manifest/receipt split in @/src/norijson/docs.md.
