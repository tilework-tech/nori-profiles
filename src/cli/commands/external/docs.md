...
GitHub-sourced skills have no registry version, so the skillset's `nori.json` records them under the shared `UNPINNED_VERSION` constant from @/src/norijson/nori.ts — the same value every registry install path records (see the manifest/receipt split in @/src/norijson/docs.md). This command's behavior predates that convention; it now names the constant rather than repeating the literal.
