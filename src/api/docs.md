...
| registrar.ts | `searchPackages()`, `getPackument()`, `downloadTarball()` | Used by nori-registry-search and nori-registry-download slash commands, and registry-search/registry-download CLI commands |
...
**Registrar API:** The registrar.ts module is a standalone API client for the Nori registrar at https://registrar.tilework.tech. Unlike other API modules that use apiRequest() with Firebase authentication, the registrar uses direct fetch() calls. It provides four functions: `searchPackages()` searches for profile packages by query string, `getPackument()` retrieves npm-compatible package metadata (packument) including version tags and tarball URLs, `downloadTarball()` downloads a package tarball (resolving to latest version if none specified), and `uploadProfile()` uploads a profile tarball with authentication. The search, packument, and download endpoints are public; the upload endpoint requires a Bearer token. The registrar API is used by the nori-search-profiles, nori-download-profile, and nori-registry-upload intercepted slash commands in @/plugin/src/installer/features/hooks/config/intercepted-slashcommands/.

**Registry Authentication:** The registryAuth.ts module handles Firebase authentication for authenticated registry operations (profile uploads). It exports `getRegistryAuthToken()` which accepts a `RegistryAuth` object (username, password, registryUrl) from the config's `registryAuths` array, authenticates with Firebase using `signInWithEmailAndPassword`, and returns a Firebase ID token. Tokens are cached per registry URL with 55-minute expiry (5 minutes before Firebase's 1-hour token expiry). Firebase app instances are also cached per registry URL to avoid reinitialization. The module uses the same Firebase project as the main Nori backend (tilework-e18c5). The `clearRegistryAuthCache()` function is exported for testing.
