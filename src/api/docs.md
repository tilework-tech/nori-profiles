...
**Type Synchronization:** The ArtifactType enum in artifacts.ts must stay synchronized with @/server/src/persistence/Artifact.ts and @/ui/src/api/apiClient/artifacts.ts to maintain contract compatibility. The type field enables distinction between user-created memories (type: 'memory'), documentation (type: 'noridoc'), webhook-ingested data (type: 'webhook'), and system-generated artifacts (type: 'summary', 'transcript', 'recipe').
