...
| registrar.ts | `searchPackages()`, `getPackument()`, `downloadTarball()` | Used by nori-search-profiles and nori-download-profile slash commands, and registry-search/registry-download CLI commands |
...
**Registrar API:** The registrar.ts module is a standalone API client for the public Nori registrar at https://registrar.tilework.tech. Unlike other API modules that use apiRequest() with Firebase authentication, the registrar uses direct fetch() calls without authentication since all endpoints are public. It provides three functions: `searchPackages()` searches for profile packages by query string, `getPackument()` retrieves npm-compatible package metadata (packument) including version tags and tarball URLs, and `downloadTarball()` downloads a package tarball (resolving to latest version if none specified). The registrar API is used by: (1) the nori-search-profiles and nori-download-profile intercepted slash commands in @/plugin/src/installer/features/hooks/config/intercepted-slashcommands/, and (2) the `registry-search` and `registry-download` CLI commands in @/plugin/src/installer/registrySearch.ts and @/plugin/src/installer/registryDownload.ts.
