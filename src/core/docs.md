...
- **`uploadSync.ts` is the one place that writes exact dependency versions into `nori.json`.** Everywhere else in the codebase, consumer-side installs record skill/subagent dependencies as the unpinned specifier `"*"` (see @/src/norijson/docs.md), since installs always take `dist-tags.latest` and the version actually on disk is recorded in each dependency's `.nori-version`. The publisher's local manifest keeps registrar-assigned exact versions after an upload; a consumer installing that same skillset normalizes them back to `"*"`. This asymmetry is intentional -- do not "fix" one side to match the other without checking both call graphs.
